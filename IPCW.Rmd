...
```{r}
cox_estimate<-function(dt,time_point, var_name){
  ...
  coxph_pred_prob = predict(cox_model, newdata = data.frame(sigma=dt$sigma,X1=dt$X1,X2=dt$X2, observed_time=rep(time_point, times=length(dt$sigma))), type="survival")
...
newnew = cox_estimate(dt,time_point, var_name)
