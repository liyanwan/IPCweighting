...
get_C <- function(distribution, num_obs,uncensor_prop, lambda,seed){
  ...
  if(distribution=='uniform'){
    # for (i in 1:seed){
    #   t=cbind(t,runif(num_obs, para1, para2))
    # }
...
```{r}
censor_cond <- function(dt,var_name){
  newdt = dt[is.na(dt$E),]
  df = subset(newdt, select = -c(event_time, censor_time))
  denominator = cox_estimate(dt,df,var_name)
  return(denominator)
}
...
censor_cond_prob = censor_cond(dt,var_name)
indices = which(is.na(dt$E))
View(dt)
cond_prob_censor_timepoint = newnew[indices]
cond_prob_censor = cond_prob_censor_timepoint/censor_cond_prob
cond_censor_error = ols_error(true_surv[indices],cond_prob_censor)
uncensor_error = ols_error(true_surv[indices],newnew[indices])
sum(cond_censor_error,uncensor_error)
