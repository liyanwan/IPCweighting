...
train_all_learners_all <- function(train_X, train_Y, X_validation = NULL, time_point, measure, range_intervals, learner_list,params_list, foldid = NULL, ...) {
  ...
  for (j in seq_along(learner_list)){
    ...
    learner_models = learner_info$model
    learner_optbins = learner_info$optbin
    learner_test_event_pred = learner_info$pred
    learner_class_name = learner_info$class_name
    model_list_with_bin[[j]] <- list(model = learner_models,
                                     bin = learner_optbins,
                                     test_event_pred = learner_test_event_pred)
    model_list_with_bin[[j]]$class_name = learner_class_name
  }
