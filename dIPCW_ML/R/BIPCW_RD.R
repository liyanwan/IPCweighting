dIPCW_ML_RD <- function(time_point,
                     X,
                     Y,
                     newX = NULL,
                     newY = NULL,
  ...
  if(include_naive){
    ...
    naive_learners_test_EP = fit_singlebin(train_X = naive_train_X, train_Y = naive_train_Y, X_validation = newX, time_point = time_point,
  ...
  model_list_with_bin <- train_all_learners_all(train_X = X, train_Y = Y, X_validation = newX,
  ...
  model_list_opt = if (include_opt) {
    train_all_learners_all(
      train_X = X, train_Y = Y, X_validation = newX,
      time_point = time_point, measure = measure,
      learner_list = learner_list, params_list = params_list,
      range_intervals = min_intervals:max_intervals,
      k = learner_k, proxy_data = proxy_data, foldid = learner_foldid)
    } else NULL
  # Coxph Survival Model
  if(include_SA){
    if(any(sapply(learner_list, identical, y = base_mars_BinnedIPCW))){
      index_match <- which(sapply(learner_list, identical, y = base_mars_BinnedIPCW))
      add_cox_test_EP = do.call(cbind,
                                lapply(index_match, function(index_cox){
                                  additive_cox(train_data = train_data, test_X = newX, time_point = time_point,
                                               cts.num = ifelse(!is.null(surv_params[[index_cox]]$cts.num), surv_params[[index_cox]]$cts.num, 5),
                                               k = ifelse(!is.null(surv_params[[index_cox]]$k), surv_params[[index_cox]]$k, 10),
                                               var_threshold = ifelse(!is.null(surv_params[[index_cox]]$var_threshold), surv_params[[index_cox]]$var_threshold, ncol(newX)))
                                }))
    }
      ...
      cox_penalized_test_EP = do.call(cbind,
                                      lapply(index_match, function(index_cox){
                                        coxph_with_penalty(
                                          ...
                                          test_X = newX,
                                          time_point = time_point,
                                          alpha_num = surv_params[[index_cox]]$alpha_num,
                                          measure = measure,
                                          lambda_grid = surv_params[[index_cox]]$lambda_values,
                                          useMin = surv_params[[index_cox]]$useMin,
    ...
    if(any(sapply(learner_list, identical, y = base_CTree_BinnedIPCW))){
      index_match <- which(sapply(learner_list, identical, y = base_CTree_BinnedIPCW))
      STree_test_EP = do.call(cbind,
                              lapply(index_match, function(index_cox){
                                ...
                                setNames(data.frame(surv_tree_event_probability), paste0("SurvTree", index_cox))
                              }))
    }
    if(any(sapply(learner_list, identical, y = base_deepnn_BinnedIPCW))){
      index_match <- which(sapply(learner_list, identical, y = base_deepnn_BinnedIPCW))
      NN_test_EP = do.call(cbind,
                           lapply(index_match, function(index_cox){
                             do.call('tune_deepSurv', c(list(X = X, Y = Y, X_validation = newX,
                                                             time_point = time_point, measure = ifelse(measure == "C-index", "cindex", "npll"),
                                                             k = learner_k, foldid = learner_foldid),
                                                        surv_params[[index_cox]]))
                           }))
    }
  }
  ...
  for(i in 1:length(model_list_with_bin)){
    ...
    } else if (col_name %in% colnames(all_model_event_probability_layer)){
      newname = paste0(col_name,'.', i)
      all_model_event_probability_layer[[newname]] = pred_test_event_probability
  ...
  if(include_SA){
    nm = c("cox_penalized_test_EP", "cox_test_EP", "add_cox_test_EP",
           "STree_test_EP", "NN_test_EP")
    env = environment()
    present = nm[sapply(nm, exists, envir = env, inherits = FALSE)]
    if (length(present)) {
      add <- mget(present, envir = env, inherits = FALSE)
      all_model_event_probability_layer <- do.call(
        cbind, c(list(all_model_event_probability_layer), unname(add))
      )
    }
  }
  ...
  for (kk in seq_len(ncol(all_model_event_probability_layer))) {
    ...
    AUC_layer[[kk]] = auc(roc(newY$E[!is.na(newY$E)], predictions[!is.na(newY$E)], quiet = TRUE))
  ...
  results_df <- setNames(
    ...
    ), c("Method","C-observed", "AUC", "-LogL", "BS", "Opt_bins"))
