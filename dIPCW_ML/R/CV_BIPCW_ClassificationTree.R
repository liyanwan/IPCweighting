  ...
  for (i in 1:nrow(cv_results)) {
    ...
    for (f in 1:k) {
      ...
      tree_model = rpart(formula,
                         ...
                         method = "class",
                         control = rpart.control(cp = ccp_alpha, minsplit = 5))
      est_test_event_single = predict(tree_model, newdata = test_data, type = "prob")[,"1"]
    ...
    if(measure == "C_index") fold_performance_measure[is.na(fold_performance_measure)] = 0.5
