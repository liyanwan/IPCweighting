...
results_total_list <- foreach(w = start_iter:end_iter, .packages = c("MASS", "dplyr","glmnet", "survival", "caret", "rpart", "earth"), .export=GlobalFunctions) %dopar% {
  set.seed(20+w)
  bin_ph <- lapply(seq(max_bin), function(bin) {
    print(bin)
    ...
    learners_knn = create.Learner("dIPCWSL.knn", tune = list(num_intervals = bin, k = neighbors_range),
                                  detailed_names = TRUE)
    learners_earth = create.Learner("dIPCWSL.earth", tune = list(num_intervals = bin, degree = c(1), nprune = c(2, 5, 10, 15, 20, 25)),
                                    detailed_names = TRUE)
  ...
  results_list = lapply(bin_ph, function(ele) ele$results_df)
  IPCWML_CVRisk = lapply(bin_ph, function(ele) ele$ML_CVRisk)
  IPCWcoef = lapply(bin_ph, function(ele) ele$coef)
  all_layer_testEP = lapply(bin_ph, function(ele) ele$all_layer_testEP)
  if(measure == "C_index"){
...
saveRDS(list(all_layer_testEP = all_layer_testEP, 
             ...
             combined_results = df, 
             IPCWcoef = IPCWcoef), paste0("SL_ph_", dist, "_", num_obs, "_", num_covariates, ".rds"))
