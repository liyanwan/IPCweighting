...
results_total_list <- foreach(w = start_iter:end_iter, .packages = c("MASS", "dplyr","glmnet", "survival", "caret", "rpart", "earth"), .export=GlobalFunctions) %dopar% {
  ...
  bin_hdpart <- lapply(seq(max_bin), function(bin) {
    print(bin)
  ...
  IPCWML_CVRisk = lapply(bin_hdpart, function(ele) ele$ML_CVRisk)
  IPCWcoef = lapply(bin_hdpart, function(ele) ele$coef)
...
saveRDS(list(all_layer_testEP = all_layer_testEP, 
             ...
             combined_results = df, 
             IPCWcoef = IPCWcoef), paste0("SL_hdpartial_", dist, "_", num_obs, "_", num_covariates, ".rds"))
