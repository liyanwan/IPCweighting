...
```{r}
naive_estimate <- function(dt, newdt, var_name, method="linear"){
  ...
  if(method == "linear"){
    ...
    surv_tree <- rpart(formula,
                 data = naive_data, 
                 method = "class",
                 control = rpart.control(minsplit = 20, cp = 0.01)) #default
    est_test_event_prob = predict(surv_tree, newdata=newdt, type = "prob")[, "1"]
    naive_newdt_survival_prob = 1-(est_test_event_prob)
  }
...
```{r}
ipcw_estimate <- function(dt,newdt, time_point, var_name, method="linear"){
  ...
  if (method == "linear"){
    ...
    surv_tree <- rpart(formula,
                 ...
                 control = rpart.control(minsplit = 20, cp = 0.01)) #default
    est_test_event_prob = predict(surv_tree, newdata=newdt, type = "prob")[,"1"]
    ipcw_survival_prob = 1-(est_test_event_prob)
  }
